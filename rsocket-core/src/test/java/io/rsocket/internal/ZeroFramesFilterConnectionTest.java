...
public class ZeroFramesFilterConnectionTest {
  ...
  public void filterAllowsNonZeroFrames() throws Exception {
    ZeroFramesFilterConnection filter =
        send(Frame.Error.from(1, new RuntimeException()), FrameType.ERROR);
  ...
  public void filterAllowsNonForbiddenFrames() throws Exception {
    ZeroFramesFilterConnection filter =
        send(Frame.Error.from(0, new RuntimeException()), FrameType.KEEPALIVE);
  ...
  public void filterDisallowsForbiddenFrames() throws Exception {
    ZeroFramesFilterConnection filter =
        send(Frame.Error.from(0, new RuntimeException()), FrameType.ERROR);
  ...
  private ZeroFramesFilterConnection send(Frame sendFrame, FrameType... forbid) {
    ...
    ZeroFramesFilterConnection filter =
        new ZeroFramesFilterConnection(new LocalDuplexConnection("conn", send, receive), forbid);
