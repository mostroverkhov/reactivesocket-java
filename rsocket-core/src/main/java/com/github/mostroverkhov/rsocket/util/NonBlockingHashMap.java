...
import static com.github.mostroverkhov.rsocket.util.UnsafeAccess.UNSAFE;
 ...
 * safe, including all update operations. {@link NonBlockingHashMap} scales substantially better
 ...
 * on a 32-way Sun US2 box, 32-way Sun Niagara box, 8-way Intel box and a 4-way Power box.
 *
 * <p>This class obeys the same functional specification as {@link Hashtable}, and includes versions
 * of methods corresponding to each method of <tt>Hashtable</tt>. However, even though all
 ...
 *
 * <p>Operations (including <tt>put</tt>) generally do not block, so may overlap with other update
 ...
 * <em>not</em> throw {@link ConcurrentModificationException}. However, iterators are designed to be
 * used by only one thread at a time.
 *
 * <p>Very full tables, or tables with high re-probe rates may trigger an internal resize operation
 * to move into a larger table. Resizing is not terribly expensive, but it is not free either;
 * during resize operations table throughput may drop somewhat. All threads that visit the table
 * during a resize will 'help' the resizing but will still be allowed to complete their operation
 * before the resize is finished (i.e., a simple 'get' operation on a million-entry table undergoing
 * resizing will not need to block until the entire million entries are copied).
 *
...
public class NonBlockingHashMap<TypeK, TypeV> extends AbstractMap<TypeK, TypeV>
   ...
   *
   * <p>The value can be retrieved by calling {@link #get} with a key that is equal to the original
   * key.
   *
  ...
  /**
   * Atomically replace newVal for oldVal, returning the value that existed there before. If the
   * oldVal matches the returned value, then newVal was inserted, otherwise not.
   *
   * @return the previous value associated with the specified key, or <tt>null</tt> if there was no
   *     mapping for the key
  ...
  public void putAll(Map<? extends TypeK, ? extends TypeV> m) {
    for (Entry<? extends TypeK, ? extends TypeV> e : m.entrySet()) put(e.getKey(), e.getValue());
  }
  ...
  private static final Object get_impl(
      final NonBlockingHashMap topmap, final Object[] kvs, final Object key) {
    ...
    while (true) {
      ...
      final Object V =
          val(kvs, idx); // Get value before volatile read, could be null or Tombstone or Prime
      ...
      if (keyeq(K, key, hashes, idx, fullhash)) {
        ...
        return get_impl(
            topmap, chm.copy_slot_and_check(topmap, kvs, idx, key), key); // Retry in the new table
      }
  ...
  // --- getk -----------------------------------------------------------------
  /**
   * Returns the Key to which the specified key is mapped, or {@code null} if this map contains no
   * mapping for the key.
   *
   * @throws NullPointerException if the specified key is null
   */
  ...
  private static final Object getk_impl(
      final NonBlockingHashMap topmap, final Object[] kvs, final Object key) {
    final int fullhash = hash(key); // throws NullPointerException if key is null
    final int len = len(kvs); // Count of key/value pairs, reads kvs.length
    final CHM chm = chm(kvs); // The CHM, for a volatile read below; reads slot 0 of kvs
    ...
    while (true) {
      ...
      if (++reprobe_cnt >= reprobe_limit(len)
        ...
        return newkvs == null
            ? null
            : getk_impl(topmap, topmap.help_copy(newkvs), key); // Retry in the new table
      }
  ...
  private static final Object putIfMatch(
    ...
    while (true) { // Spin till we get a Key slot
      ...
      if (keyeq(K, key, hashes, idx, fullhash)) break; // Got it!
    ...
    while (true) { // Spin till we insert a value
      ...
      if (newkvs != null)
        return putIfMatch(
            topmap, chm.copy_slot_and_check(topmap, kvs, idx, expVal), key, putval, expVal);
  ...
  private static final class CHM<TypeK, TypeV> {
    ...
    private final Object[] resize(NonBlockingHashMap topmap, Object[] kvs) {
      ...
      if (sz >= (oldlen >> 2)) { // If we are >25% full of keys then...
        ...
        if (4L * sz >= ((oldlen >> 20) != 0 ? 3L : 2L) * oldlen)
          newsz =
              oldlen
                  << 2; // Double double size, so new table will be between %12.5 (18.75%) and 25%
        // (25%)
      }
    ...
    // of the top position.
    private final void help_copy_impl(
        NonBlockingHashMap topmap, Object[] oldkvs, boolean copy_all) {
    ...
    private final Object[] copy_slot_and_check(
        NonBlockingHashMap topmap, Object[] oldkvs, int idx, Object should_help) {
      assert chm(oldkvs) == this;
      Object[] newkvs = _newkvs; // VOLATILE READ
    ...
    private final void copy_check_and_promote(
        NonBlockingHashMap topmap, Object[] oldkvs, int workdone) {
      assert chm(oldkvs) == this;
    ...
    private boolean copy_slot(
        NonBlockingHashMap topmap, int idx, Object[] oldkvs, Object[] newkvs) {
  ...
  // --- entrySet ------------------------------------------------------------
  // Warning: Each call to 'next' in this iterator constructs a new SimpleImmutableEntry.
  private class SnapshotE implements Iterator<Entry<TypeK, TypeV>> {
    ...
    public Entry<TypeK, TypeV> next() {
      _ss.next();
      return new SimpleImmutableEntry<>((TypeK) _ss._prevK, _ss._prevV);
    }
   ...
   * subsequent to construction.
   *
   * <p><strong>Warning:</strong> the iterator associated with this Set requires the creation of
   * {@link Entry} objects with each iteration. The {@link NonBlockingHashMap} does not normally
   * create or using {@link Entry} objects so they will be created soley to support this iteration.
   * Iterating using {@link #keySet} or {@link #values} will be more efficient.
   */
  @Override
