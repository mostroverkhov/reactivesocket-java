 ...
 * substatially better than {@link java.util.concurrent.ConcurrentHashMap} for high update rates,
 * even with a large concurrency factor. Scaling is linear up to 768 CPUs on a 768-CPU Azul box,
 * even with 100% updates or 100% reads or any fraction in-between. Linear scaling up to all cpus
 * has been observed on a 32-way Sun US2 box, 32-way Sun Niagra box, 8-way Intel box and a 4-way
 * Power box.
 *
 * <p><strong>The main benefit of this class</strong> over using plain {@link NonBlockingHashMap}
 * with {@link Long} keys is that it avoids the auto-boxing and unboxing costs. Since auto-boxing is
 * <em>automatic</em>, it is easy to accidentally cause auto-boxing and negate the space and speed
 * benefits.
 *
 ...
 *
 * <p>Very full tables, or tables with high reprobe rates may trigger an internal resize operation
...
public class NonBlockingHashMapLong<TypeV> extends AbstractMap<Long, TypeV>
  ...
  static { // <clinit>
    ...
    _chm_offset = UnsafeAccess.UNSAFE.objectFieldOffset(f);
    ...
    _val_1_offset = UnsafeAccess.UNSAFE.objectFieldOffset(f);
  }
  ...
  private static final class CHM implements Serializable {
    ...
    private boolean CAS_key(int idx, long old, long key) {
      return UnsafeAccess.UNSAFE.compareAndSwapLong(_keys, rawIndex(_keys, idx), old, key);
    }

    private boolean CAS_val(int idx, Object old, Object val) {
      return UnsafeAccess.UNSAFE.compareAndSwapObject(_vals, rawIndex(_vals, idx), old, val);
    }
    ...
    private void print() {
      for (int i = 0; i < _keys.length; i++) {
        long K = _keys[i];
        if (K != NO_KEY) print_impl(i, K, _vals[i]);
      }
    ...
    private Object get_impl(final long key) {
      ...
      while (true) {
        ...
        final Object V =
            _vals[idx]; // Get value before volatile read, could be null or Tombstone or Prime
  ...
  private class SnapshotE implements Iterator<Entry<Long, TypeV>> {
    ...
    public Entry<Long, TypeV> next() {
      _ss.next();
      return new SimpleImmutableEntry<>(_ss._prevK, _ss._prevV);
    }
   ...
   * Iterating using {@link #keySet} or {@link #values} will be more efficient. In addition, this
